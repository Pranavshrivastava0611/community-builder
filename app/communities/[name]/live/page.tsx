...
function CommunityLiveContent() {
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-orange-500/30 font-sans">
            ...
            <main className="max-w-[1800px] mx-auto px-4 md:px-10 pt-4 md:pt-6 pb-20">
                <motion.header
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mb-6 md:mb-8 flex flex-col md:flex-row items-center justify-between gap-6"
                >
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <button
                            onClick={() => selectedStream || isBroadcasting ? (setSelectedStream(null), setIsBroadcasting(false)) : router.back()}
                            className="bg-white/5 hover:bg-white/10 p-2.5 md:p-3 rounded-xl md:rounded-2xl border border-white/10 transition-all group"
                        >
                        ...
                        <div className="flex-1">
                            <h1 className="text-xl sm:text-2xl md:text-3xl font-black uppercase tracking-tighter truncate max-w-[200px] sm:max-w-none">
                                {community.name} <span className="text-orange-500">Live</span>
                            </h1>
                            <div className="flex items-center gap-2 text-[9px] md:text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5 md:mt-1">
                                <span className="text-orange-500">Sector A-1</span>
                    ...
                    {!isBroadcasting && !selectedStream && (
                        <button
                            ...
                            className="w-full md:w-auto bg-orange-600 hover:bg-orange-500 text-white px-8 py-3.5 md:py-3 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] md:text-[11px] border border-orange-400/30 transition-all shadow-[0_0_20px_rgba(234,88,12,0.3)] active:scale-95"
                        >
                            <span className="hidden sm:inline">Start Transmission</span>
                            <span className="sm:hidden text-sm">Start Broadcast</span>
                        </button>
                    )}
                </motion.header>
                ...
                {(isBroadcasting || selectedStream) ? (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <LiveStreamingRoom
                            ...
                            roomName={selectedStream?.room_name || (isBroadcasting ? `${community.id}-${publicKey?.toBase58()}` : undefined)}
                        />
                    </div>
                ) : (
